...
export default function users() {
    ...
    const [currentStep, setCurrentStep] = useState(2); // 1: Asset Requisition, 2: Assign Organization
    ...
    const [assetRequisitionId, setAssetReqId] = useState('');
    ...
    const saveForm = async e => {
        ...
        e.preventDefault();
        try {
            const assestrequisition = {requisition_id: assetRequisitionId, requisition_date: assetRequisitionDate, requisition_status: requisitionStatus, items: items}
    ...
    const submitForm = async e => {
        ...
        try {
            if (status === 'saved') {
                const assestrequisitionupdatedata = {requisition_id: assetRequisitionId, requisition_status: status}
                ...
                const assestrequisition = {requisition_id: assetRequisitionId, requisition_date: assetRequisitionDate, requisition_status: status, items: items}
                addAssestRequisition(assestrequisition)
                .unwrap()
                .then((response) => {
                    console.log("New node added:", response);
    ...
    const formattedFormData = formatKeys({ assetRequisitionId, assetRequisitionDate, items: formatKeys(items) });
                        ...
                        {isOpenAddForm && (
                            <div className="fixed top-0 left-0 z-50 w-full h-full flex items-center justify-center" style={{ marginLeft:0 }}>
                                ...
                                <div className="bg-white w-4/5 p-4 rounded-lg z-50 dark:bg-[#1e1e1e]">
                                                ...
                                                <div className="grid gap-6 mb-6 md:grid-cols-2">
                                                    <div className='flex'>
                                                        ...
                                                        <span className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                                            {assetRequisitionId}
                                                            <input type="hidden" value={assetRequisitionId} />
                                                        </span>
                                                    </div>
                                    ...
                                    {currentStep === 2 && (
                                        <Requisitionsapproval formData={formattedFormData} modelData={modelTopic} requestType={requestType}/>
                                    )}
                                </div>
                            </div>
                        )}
