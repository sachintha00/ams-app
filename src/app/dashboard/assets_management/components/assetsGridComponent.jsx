...
import ThumbnailImage from "./imagedisplay/ThumbnailImage";
          ...
          {data.map((Assets) => {
            return(
              <div className="p-[10px] w-full my-5 bg-white border border-gray-200 rounded-lg shadow dark:bg-[#1e1e1e] dark:border-gray-700">
                {/* <img
                  className="rounded"
                  src={`${process.env.NEXT_PUBLIC_API_URL}storage/${Assets.thumbnail_image[0].replace('public/', '')}`}
                  alt={`${Assets.model_number}`}
                /> */}
                <ThumbnailImage image={Assets.thumbnail_image[0]} name={Assets.sub_category_name}/>
                <div>
