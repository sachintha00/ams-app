...
import { useAssestListQuery, useDeleteAssetsMutation } from '@/app/_lib/redux/features/assetsmanagement/assets_management_api';

function DeleteAssetsForm({ }) {
    const dispatch = useDispatch()
    const { refetch } = useAssestListQuery();
    const { id } = useSelector((state) => state.popupModel);
    const { value } = useSelector((state) => state.popupModel);
    console.log(id);
    const [DeleteError, setDeleteError] = useState("");

    const [deleteRoles, setDelete] = useState("");
    ...
    const submitDeleteForm = async (e) => {
      ...
      try {
        if (deleteRoles === value.serial_number) {
          const Asset = { ID: value.id };
          console.log(Asset);
          deleteAssets(Asset)
            .unwrap()
            .then((response) => {
          ...
          setDeleteError("Input does not match the serial number of the serial number");
          const timer = setTimeout(() => {
            setDeleteError("");
          }, 5000); // Adjust the duration (in milliseconds) as needed
          return () => clearTimeout(timer);
    ...
    return (
        <>
            ...
            <h3 className="text-base w-[90%] text-gray-900 dark:text-white">
                If you want Delete <span className='font-semibold'>Serial Number:{value.serial_number}</span> {value.sub_category_name}, please enter <span className='font-semibold'>"{value.serial_number}"</span> on below feild
            </h3>
            </div>
            {/* Modal body */}
...
export default DeleteAssetsForm;