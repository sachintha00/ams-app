...
const SupplierListDrodwon = ({ handleSupplierSelect, searchInput, selectedSupplier, procuremenSelectedSuppliers }) => {
    const [filteredSupplares, setFilteredSupplares] = useState([]);

    useEffect(() => {
        if (procuremenSelectedSuppliers) {
            const filtered = procuremenSelectedSuppliers.filter(supplair => supplair.name.toLowerCase().includes(searchInput.toLowerCase()));
            setFilteredSupplares(filtered);
        }
    }, [procuremenSelectedSuppliers, searchInput]);
