...
import { useDeleteRoleMutation, useRolesListQuery } from '@/app/_lib/redux/features/role/role_api';

function DeleteRolesForm({ }) {
    const dispatch = useDispatch()
    const { refetch } = useRolesListQuery();
    ...
    return (
        <>
            ...
            <h3 className="text-base w-[90%] text-gray-900 dark:text-white">
                If you want Delete <span className='font-semibold'>{value.name}</span> Role, please enter <span className='font-semibold'>"{value.name}"</span> on below feild
            </h3>
            </div>
            {/* Modal body */}
...
export default DeleteRolesForm;