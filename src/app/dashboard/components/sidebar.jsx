...
const Sidebar = ({ collapsed, onToggleSidebar }) => {
  ...
  useEffect(() => {
    if (!isLoading && !isError && SidebarList && SidebarList.sidebaritem) {
      const menuItems = organizationTableDataToTreeStructure(
  ...
  return (
    <aside
      ...
      className={`sidebar ${
        collapsed ? "collapsed" : ""
      } fixed top-0 left-0 z-40 w-[60px] h-screen transition-transform -translate-x-full sm:translate-x-0 sidebarmain mt-[77px] border-r border-[#D7DAD3] dark:border-[#3c4042]`}
      aria-label="Sidebar"
    >
        ...
        <ul className="space-y-2 font-medium">
          <li>
            ...
            >
              ...
              <span className={`${collapsed ? "block" : "hidden"} ms-3`}>
                Dashboard
              </span>
              <span
                className={`${collapsed ? "hidden" : ""} ${
                  hoveredItem === "Dashboard" ? "block" : "hidden"
                } shadow flex absolute ml-6 p-2.5 text-gray-900 rounded-lg dark:text-white bg-gray-50 dark:bg-[#3c4042] w-40`}
              >
                Dashboard
              </span>
            </Link>
          ...
          {menuItems.map((sidebaritem) => (
            <li
              key={sidebaritem.id}
              onClick={() => handleMenuItemClick(sidebaritem.id)}
              onMouseEnter={() => setHoveredItem(sidebaritem.menuname)}
              onMouseLeave={() => setHoveredItem(null)}
              className={`${collapsed ? "" : "flex"}`}
            >
              <Link
              ...
              <Link href={sidebaritem.menulink}>
                <span
                  className={`${collapsed ? "hidden" : ""} ${
                    hoveredItem === sidebaritem.menuname ? "block" : "hidden"
                  } flex shadow absolute p-2.5 text-gray-900 rounded-lg dark:text-white bg-gray-50 dark:bg-[#3c4042] w-44 justify-between items-center`}
                >
                  {sidebaritem.menuname}
                  <svg
                    className="w-3 h-3"
              ...
              {sidebaritem.children && (
                <ul
                  ...
                  className={`${collapsed ? "hidden" : "block"} ${
                    hoveredItem === sidebaritem.menuname ? "block" : "hidden"
                  } shadow absolute p-2.5 text-gray-900 rounded-lg dark:text-white bg-gray-50 dark:bg-[#3c4042] w-44 justify-between items-center ml-12 mt-10`}
                >
                  {sidebaritem.children.map((submenu1) => (
                    <li key={submenu1.id}>
                      <Link
                        href={submenu1.menulink}
                        className="flex items-center justify-between w-full p-2 text-gray-900 transition duration-75 rounded-lg group dark:text-white"
                      >
                        {submenu1.menuname}
                      </Link>
                    </li>
              ...
              {sidebaritem.children && activeMenuItem === sidebaritem.id && (
                <ul
                  id="dropdown-example"
                  className={`${collapsed ? "block" : "hidden"} py-2 space-y-2`}
                >
                  {sidebaritem.children.map((submenu1) => (
                    <li key={submenu1.id}>
                      <Link
