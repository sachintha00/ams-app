              ...
              {Array.isArray(item[header]) 
                ? item[header].map((subItem, subIndex) => (
                    <div key={subIndex} className="flex">
                      {typeof subItem === 'object' ? renderObjData(subItem, level + 1) : subItem}
                    </div>
                  ))
                : (typeof item[header] === 'object' ? renderObjData(item[header], level + 1) : item[header])
              }
            </td>
          ))
        );
                  ...
                  <tbody>
                    <tr className='odd:bg-white odd:dark:bg-[#1e1e1e] even:bg-gray-50 even:dark:bg-[#3c4042] border-b dark:border-gray-700'>
                      <td className="border px-4 py-2">
                        {typeof item === 'object' ? renderObjData(item, level + 1) : item}
                      </td>
                    </tr>
                  </tbody>
                </table>
